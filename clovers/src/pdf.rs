...
impl<'a> PDF<'a> {
    pub fn value(&self, direction: Vec3, time: Float, rng: SmallRng) -> Float {
        match self {
    ...
    pub fn generate(&self, rng: SmallRng) -> Vec3 {
        match self {
...
impl CosinePDF {
    ...
    pub fn value(&self, direction: Vec3, _time: Float, _rng: SmallRng) -> Float {
...
impl<'a> MixturePDF<'a> {
    ...
    pub fn value(&self, direction: Vec3, time: Float, rng: SmallRng) -> Float {
        0.5 * self.pdf1.value(direction, time, rng.clone())
            + 0.5 * self.pdf2.value(direction, time, rng.clone())
    }

    pub fn generate(&self, mut rng: SmallRng) -> Vec3 {
        if rng.gen::<bool>() {
...
impl ZeroPDF {
    ...
    pub fn value(&self, _direction: Vec3, _time: Float, _rng: SmallRng) -> Float {
        0.0
    }

    pub fn generate(&self, _rng: SmallRng) -> Vec3 {
